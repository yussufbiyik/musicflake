...
export class WeatherService {
  openmateoApiUrl = (latitude:number, longtitude:number) => {return `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longtitude}&hourly=temperature_2m,relativehumidity_2m,weathercode`}
  ...
  getWeatherCode(): Number | String{
    try {
      ...
      var currentTimeWeathercode: number  = -999;
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        this.http.get<any>(this.openmateoApiUrl(latitude,longitude)).subscribe((resp:any) => {
          ...
          currentTimeWeathercode = resp.hourly.weathercode[currentTimeWeatherIndex]
